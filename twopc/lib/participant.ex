defmodule TPC.Participant do
  ...
  def allocate(allocater_pid, crdt_pid, lease_amount) do
    ...
    GenServer.cast(TPC.Network.coordinator(), {:start, crdt_pid, {:allocate, lease_amount, allocater_pid}})
  end

  def deallocate(deallocater_pid, crdt_pid, lease_amount) do
    # Body = {:deallocate, amount, deallocater_pid}
    GenServer.cast(TPC.Network.coordinator(), {:start, crdt_pid, {:deallocate, lease_amount, deallocater_pid}})
  end
  ...
  def handle_cast({:new_follower, new_follower_name,  new_follower_pid}, {name, followers, votes} = _state) do
    IO.puts("Coordinator #{inspect(TPC.Network.coordinator())} added follower with PID #{inspect(new_follower_pid)}")
  ...
  def handle_cast({:start, crdt_pid, body}, {name, followers, _votes} = state) do
    ...
    if name == TPC.Network.coordinator() do
      # Include leader
      followers_with_leader = Map.put(followers, TPC.Network.coordinator(), TPC.Network.coordinator())
      ...
      Enum.each(followers_with_leader, fn {name, pid} ->
        ...
        IO.puts("Coordinator #{inspect(TPC.Network.coordinator())} sent a prepare request to follower #{inspect(pid)}")
      end)
    else
  ...
  def handle_cast({:prepare_request, crdt_pid, body}, state) do
    ...
    vote = case GenServer.call(crdt_pid, {:prepare, body}) do
      :ok ->
        ...
        TPC.Network.VoteMessages.vote_commit()
      :abort ->
        # Send not ok to coordinator
        TPC.Network.VoteMessages.vote_abort()
    end

    GenServer.cast(TPC.Network.coordinator(), {:vote_response, self(), vote, body})
    IO.puts("Follower #{inspect(self())} sent a vote response to coordinator #{inspect(TPC.Network.coordinator())}")
    {:noreply, state}
  end
  ...
  def handle_cast({:vote_response, sender_pid, vote, body}, {name, followers, votes} = _state) do
    ...
    if vote == TPC.Network.VoteMessages.vote_commit() do
      ...
      if Map.size(new_votes) == Map.size(followers) do
        ...
        # Commit on leader
        GenServer.cast(TPC.Network.coordinator(), {:commit, sender_pid, body})
        # Commit on followers
